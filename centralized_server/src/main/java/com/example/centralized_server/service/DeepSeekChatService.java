...
public class DeepSeekChatService {
    ...
    public TrademarkAnalysis checkTrademark(String brandNameToCheck) {
        ...
        // or that your DeepSeek model reliably adheres to JSON instructions from the prompt.

        try {
            TrademarkAnalysis analysis = chatClient.prompt()
                    .system(systemPromptForTrademark)
                    .user(userQuery)
                    .call()
                    .entity(TrademarkAnalysis.class);
            return analysis;
        } catch (Exception e) {
            // Log the error for debugging
            System.err.println("Error parsing JSON from AI or calling API: " + e.getMessage());
            // Return an error object or throw a custom exception
            // Example: return a null or specific error DTO
            return new TrademarkAnalysis(
                    List.of(),
                    "Error",
                    "Could not receive or parse the response from the AI. Please try again. Error details: " + e.getMessage()
            );
        }
    }
}
